...
class LX16A:
    ...
    def move(
        self, angle: float, time: int = 0, relative: bool = False, wait: bool = False, wait_to_complete: bool = False
    ) -> None:
        ...
        if wait_to_complete:
            print('Waiting to complete before moving on to other commands')
            completing = True
            init_angle  = self.get_physical_angle()
            start = t.time()
            
            while completing:
                if abs(degree-init_angle)/100 < time/1000:
                    if t.time()-start > time/1000+0.25:
                        print('Completed Command')
                        completing = False
                else:
                    if t.time()-start > abs(degree-init_angle)/100+.25:
                        print('Completed Command')
                        completing = False
