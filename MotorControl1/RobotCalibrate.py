...
class RobotCalibrate():
	...
	def __init__(self):
		...
		self.servo1_start_pos = 65.28 #right thigh
		self.servo2_start_pos = 96.0 #right shin
		self.servo3_start_pos = 58.8 #right ankle

		#LEFT LEG STARTING POSITIONS
		self.servo4_start_pos = 65.28 #left thigh
		self.servo5_start_pos = 148.8 #left shin
		self.servo6_start_pos = 63.84 #left ankle
		...
		#JOINT DEFINITIONS (FOR KINEMATICS)
		self.A_x, self.A_y = 0, 0 #thigh motor joint
		self.B_x, self.B_y = 0, -self.l1 #tendon motor joint

		self.theta_1 = (pi/180)*(self.servo1_start_pos-65) #thigh motor angle with the axis pointing up and down
		self.theta_2 = (pi/180)*(self.servo2_start_pos-135) #tendon motor angle with the axis pointing up and down
	...
	def calibrate(self):
		...
		# self.servo1.move(66.24, time=750, wait_to_complete=False)
		# self.servo2.move(102.72, time=750, wait_to_complete=False)
		# self.servo3.move(65.28, time=750, wait_to_complete=True)

		# self.servo4.move(58.56, time=750, wait_to_complete=False)
		# self.servo5.move(138.48, time=750, wait_to_complete=False)
		# self.servo6.move(63.36, time=750, wait_to_complete=True)
	...
	def check_motors(self):
		...
		servo4_goal_pos = servo4_init_pos
		...
		while dt < pi:
			...
			print('Ankle Motor =', servo3_next)

			self.servo4.move(self.servo4_start_pos, wait_to_complete=False)
			self.servo5.move(self.servo5_start_pos, wait_to_complete=False)
			self.servo6.move(self.servo6_start_pos, wait_to_complete=False)

			self.servo1.move(servo1_next, time=50)
			self.servo2.move(servo2_next, time=50)
			self.servo3.move(servo3_next, time=50, wait_to_complete=True)

			# self.servo4.move(cos(dt) * diff4 + servo4_init_pos)
			# self.servo5.move(sin(dt) * diff5 + servo5_init_pos)
			# self.servo6.move(cos(dt) * diff6 + servo6_init_pos)
